...
%% 4. 針對 Loopback 的平方相位同步法 (Squaring Loop)
fprintf('計算靜態相位偏移...\n');

% 1. 把基頻訊號平方。這會把左下到右上的斜線，全部翻到同一邊，並把角度放大兩倍
rx_squared = rx_baseband .^ 2;

% 2. 取整個時段的平均值，然後算角度，再除以 2 換算回原本的相位角 (theta)
theta_est = angle(mean(rx_squared)) / 2;

% 3. 將原始訊號乘上反向的固定角度，整條斜線就會被完美轉平！
rx_sync = rx_baseband .* exp(-1j * theta_est);
