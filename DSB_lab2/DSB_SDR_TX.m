...
Loop_Duration = 20; % 設定為 30 秒以確保不超過硬體記憶體極限
...
%% --- 2. 參數與重採樣 ---
fs = 960e3;              % 通訊採樣率：0.96 MHz (配合 SDR 硬體)
Target_Freq_MHz = 433;   % SDR 硬體中心頻率 (RF)

fprintf('正在進行重採樣至 %.2f MHz... (請稍候)\n', fs/1e6);

% 將音訊提升到通訊採樣率，並直接轉置為單一 row (1 x N)
mt = resample(y, fs, fs_audio).';

% 【絕對關鍵】移除直流偏壓 (Mean = 0)
% 若不移除，SDR 混頻後會在 433MHz 正中央產生強烈的載波洩漏 (Carrier Leakage)
mt = mt - mean(mt);

% 音量正規化與調整振幅 (避免 DAC 破音，設定最大振幅為 0.8)
