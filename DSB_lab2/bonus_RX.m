...
% --- 解決 Loop back 相位偏移 ---
% 1. 將訊號平方以消除音樂正負號的影響 (m(t)^2 永遠為正)，剩下的角度就是 2*theta
rx_squared = rx_filtered .^ 2;

% 2. 估算這段時間的平均相位差 (除以2是因為平方把角度放大了兩倍)
theta_est = angle(mean(rx_squared)) / 2;

% 3. 將訊號乘上 exp(-j*theta) 進行相位旋轉，把它對齊回 I 通道
rx_sync = rx_filtered .* exp(-1j * theta_est);

% 4. 此時再取實部，就不會因為相位差而衰減了
audio_demod = real(rx_sync);
