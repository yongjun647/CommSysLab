...
% [Integrator] 離散累加近似積分(∑m[k]*dt)
dt_proc = 1/fs_proc;
int_m_n = cumsum(m_n) * dt_proc;

% [Multiplier] 產生原始相位 \Theta[n]=2*pi*k_f*\Sigma m*dt
theta_n = (2 * pi * FM_Dev) * int_m_n;

% ========================================================
% 依照您的指示：將開頭與結尾的相位強制歸零
% ========================================================
zero_time = 0.1; % 設定歸零的時間長度，例如 0.1 秒 (可自行微調)
zero_samples = round(zero_time * fs_proc); % 換算成取樣點數

% 防呆機制：確保歸零點數不會超過訊號總長度的一半
if zero_samples > length(theta_n)/2
    zero_samples = floor(length(theta_n)/2);
end

% 將陣列開頭與結尾的相位數值強制覆寫為 0
theta_n(1:zero_samples) = 0;
theta_n(end-zero_samples+1:end) = 0;
% ========================================================

% [e^j()] 產生數位複數基頻 I[n]+jQ[n]
% 當 theta_n 為 0 時，exp(0) = 1 + 0j，即發射純載波
iq_base_n = exp(1j * theta_n);

% [模擬 DAC] 透過 10 倍重採樣,讓數位訊號準備送入SDR 物理層
